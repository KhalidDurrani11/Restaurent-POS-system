...
const PosScreen: React.FC = () => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      await addSale(saleItems, user.id, paymentMethod);
      setReceiptNumber(receiptId);
      setReceiptItems(saleItems);
      setCart([]);
      setShowReceipt(true);
    } catch (e) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 min-h-0 flex-1">
      {showReceipt && receiptItems.length > 0 && (
        <Receipt
          cart={receiptItems}
          total={receiptItems.reduce((acc, item) => acc + item.price * item.quantity, 0)}
          paymentMethod={paymentMethod}
